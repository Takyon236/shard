...
* `profile_repo` — the static facts. This is preflight (the integration guideb): what the
...
#: How many memory-unsafe SOURCE paths to keep as a sample. A sample, not an inventory: the point is to
...
class TargetProfile:
    ...
    is read by something today. the integration guideb names further measurements — pull-request
    ...
    #: A SAMPLE of the memory-unsafe source files found, repo-relative, capped at
    #: `_MAX_NATIVE_SAMPLE`. Headers are excluded on the same rule `memory_unsafe` uses. Exists so a
...
def profile_repo(root, *, max_files: int = MAX_WALK_FILES, markers: dict | None = None) -> TargetProfile:
    ...
    for dirpath, dirnames, filenames in os.walk(root, onerror=lambda _e: None):
        ...
        for name in filenames:
            ...
            elif lang in _MEMORY_UNSAFE and len(native) < _MAX_NATIVE_SAMPLE:
                native.append(f"{rel_dir}/{name}".lstrip("/"))
            path = here / name
...
#
# Simple-safe and living HERE, beside the discovery it reads (`fuzz_harnesses`), because two readers
# need the same two facts — which targets exist, and which are built — and only one of them may reach
# the other: the separate package derives the workdir and may import from `shard.target`, while `preflight`
...
#
# No new scanning. `_FUZZ_ENTRY` already matches `LLVMFuzzerTestOneInput`, so `profile.fuzz_harnesses`
# ALREADY holds these paths — 23 of them in facebook/zstd, 13 in libgit2, measured 2026-08-21. The
# defect was never discovery; it was that nothing downstream could use what discovery found. This is
# the narrowing, and it lives here for the same reason `cargo_fuzz_targets` does: `preflight` must be
...
#:
#: **FUZZ HARNESSES ARE DELIBERATELY NOT HERE, and the reason is the whole soundness argument.**
#: `_FUZZ_ENTRY` matches `LLVMFuzzerTestOneInput`, `fuzz_target!` and Go's `FuzzXxx` — every one of
#: which must be COMPILED before it can run. `adjudicate` executes `bash -- <entry> <input>`; it does
...
def demonstrability(root, declared: str = "") -> dict:
    ...
    **THE PRODUCT'S CENTRAL CLAIM HAS A PRECONDITION AND `preflight` DID NOT CHECK IT.** Shard reports
    ...
    `preflight` is the instrument the integration guideb designates to answer *"will this work
...
def fork_workflow_template() -> str:
    """A ready-to-commit `.github/workflows/shard-fork.yml` — the fork-PR workflow, not transcribed.

    **THE RECIPE EXISTS AND THE FAILURE MODE IS THE TRANSCRIPTION.** Reviewing a fork's pull
    request needs a `workflow_run` workflow, because GitHub withholds secrets from the
    `pull_request` event a fork triggers — the README carries the recipe, and it is a file a
    customer copies by hand. Three lines in it are load-bearing, and a copy-paste error in any of
    the three produces no error message at all:

    * `repository:`/`ref:` under checkout — without them `workflow_run` starts on the BASE
      branch, so the run reviews YOUR OWN CODE and reports a clean result for a pull request
      nobody read. That is the green-check-that-reviewed-nothing this product refuses everywhere
      else, rebuilt by one missing `with:` block.
    * `fetch-depth: 0` — the same failure the main workflow's comment names: one commit checks
      out nothing to diff against, and the run reports "no changed files in scope" and exits 0.
    * `slug:` — without it the report, the alerts and the SARIF name YOUR repository for a review
      of somebody else's code.

    A fixed skeleton emitted by the product cannot drop a line the way a transcription can, and
    `--fork-workflow > .github/workflows/shard-fork.yml` is one command. PRINTED RATHER THAN
    WRITTEN for the same reason `entry_template` is: preflight is the command a customer runs to
    decide whether to adopt this product at all, and committing the file is what makes it code
    review rather than tool output.

    The one thing the skeleton CANNOT know is the name of the fork-triggered build it waits for
    (`workflows: [ci]`), and that line carries its own CHANGE-ME comment rather than a guessed
    default kept silently wrong.
    """
    return '''# Shard: review a pull request opened from a FORK, where GitHub withholds secrets from the
# `pull_request` event. Generated by `shard preflight --fork-workflow` — commit it at
# .github/workflows/shard-fork.yml, and read the three lines named below before you edit it.
#
# THE THREE LINES THAT MAKE THIS WORK, because a copy-paste error in any of them produces a
# GREEN CHECK THAT REVIEWED NOTHING rather than an error:
#   repository:/ref:  without them `workflow_run` starts on the BASE branch — your own code,
#                     reviewed clean, for a pull request nobody read.
#   fetch-depth: 0:   one commit checks out nothing to diff against; the run reports "no
#                     changed files in scope" and exits 0.
#   slug:             without it the report and the alerts name YOUR repository for a review
#                     of somebody else's code.
name: shard-fork
on:
  workflow_run:
    workflows: [ci]          # <- CHANGE to the name of the fork-triggered build this waits for
    types: [completed]
...
jobs:
  review:
    ...
    steps:
      # CHECK OUT THE FORK'S HEAD EXPLICITLY. `workflow_run` starts on the BASE branch, so
      # without a ref this reviews your own code and reports a clean result for a pull request
      # nobody read.
      - uses: actions/checkout@v4
        with:
          ...
          fetch-depth: 0
          # A checkout of somebody else's repository leaves its token in the git context unless
          # told not to. Nothing after this step needs it.
          persist-credentials: false

      - uses: Takyon236/shard@v2
        env:
          # THE SECRET ITSELF, and the only place it appears — never as an `with:` input.
        ...
        with:
          ...
          # for a review of somebody else's code — which is why the SARIF upload refuses
          # outright when the reviewed repository is not the token's.
          slug: ${{ github.event.workflow_run.head_repository.full_name }}

# A witness entry point EXECUTES code from the fork, under a token the `pull_request` job did
# not have. Treat `witness_entry` on fork pull requests as a decision, not a default — add it
# to the `with:` block above only after that decision is made.
'''


# --- what a run will cost the customer, PRODUCT-AUDIT P6 ---------------------------------------------

#: **EVERY LIVE DIFF RUN THIS PROJECT HAS METERED, and the calibration set is exactly this small.**
#: the integration guideb designates preflight the pricing instrument and preflight said nothing
...
def probe_runtimes(languages) -> dict:
    ...
    the box cannot run their language. the integration guideb makes preflight the instrument that
