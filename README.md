...
> **Version:** These pages document `v4.0.2`. Existing v3 users should use the
...
After `v4.0.2` is published, this profiles the Shard checkout without a model call or API key:

```bash
set -euo pipefail
demo_dir='shard-v4.0.2'
test ! -e "$demo_dir"
git clone --branch v4.0.2 --depth 1 https://github.com/Takyon236/shard "$demo_dir"
cd "$demo_dir"
python3 -m venv .venv
. .venv/bin/activate
